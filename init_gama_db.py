...
def init_db():
    ...
    # 3. Anadir columna gama_asignada si no existe
    try:
    ...
    # 4. Asignar gama a cada item
      cursor.execute("SELECT id, categoria, unitario_mediana FROM item_master")
    ...
    for item_id, item_cat, unitario in items:
              ...
              cursor.execute("SELECT medium_min, premium_min, luxury_min, luxury_plus_min FROM gama_ranges WHERE categoria = ? LIMIT 1", (item_cat,))
              row = cursor.fetchone()
              if row:
    ...
    conn.commit()
    conn.close()
    print(f"✅ BD inicializada - {len(items)} items asignados")
